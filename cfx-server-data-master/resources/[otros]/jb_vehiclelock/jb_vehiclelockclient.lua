				...
				if not table.empty(VehicleList) then
					...
					if isvehiclefound then
						if (islocked == 1)then
							...
							Notify("Vehículo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ esta ~r~cerrado ~w~.")
							-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
						else
							SetVehicleDoorsLocked(vehicle,1)
							Notify("Vehículo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ esta ~g~abierto ~w~.")
					...
					SetVehicleDoorsLocked(vehicle, 2)
					Notify("Vehículo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ est ~r~cerrado ~w~.")
					-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
					table.insert(VehicleList, {plate = vehicleplate})
				end
			else
				Notify("~r~Ningún vehículo para bloquear. 3")
			end
		else
			if not table.empty(VehicleList) then
					...
					if isvehiclefound then
						if not foundclostestvehicle then 
							if (islockedHandle == 1)then
								...
								Notify("Vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ esta ~r~cerrado ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
							else
								SetVehicleDoorsLocked(vehicleHandle,1)
								Notify("Vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ est ~g~abierto ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
							end
						else
							if (islockedclostestvehicle == 1)then
								SetVehicleDoorsLocked(clostestvehicle, 2)
								Notify("Vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(clostestvehicle))) .. "~w~ est ~r~cerrado ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
								foundclostestvehicle = false
							else
								SetVehicleDoorsLocked(clostestvehicle,1)
								Notify("Vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(clostestvehicle))) .. "~w~ est ~g~abierto ~w~.")
								-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
								foundclostestvehicle = false
							end
						...
						Notify("El vehículo que quieres abrir, no es ~r~ tuyo~w~.")
					end
				else
		...
		if DoesEntityExist(vehicleHandle) then
			if vehicleHandleplate ~= nil then
				...
				if isvehiclefound then
					if (islockedHandle == 1)then
						SetVehicleDoorsLocked(vehicleHandle, 2)
						Notify("Vehículo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ esta ~r~cerrado ~w~.")
						-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
					else
						SetVehicleDoorsLocked(vehicleHandle,1)
						Notify("Vehículo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ esta ~g~abierto ~w~.")
						-- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
					end
				else
					Notify("El vehículo que quieres abrir, no es ~r~tuyo~w~.")
