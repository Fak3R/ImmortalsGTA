					...
					if closestDistance ~= -1 and closestDistance <= 3 then
						...
						if not IsPedSittingInAnyVehicle(closestPed) then
							if PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name] ~= nil and PersonalMenu.ItemSelected.count > 0 then
								TriggerServerEvent('esx:giSalvadorveInventoryItem', GetPlayerServerId(closestPlayer), 'item_standard', PersonalMenu.ItemSelected.name, PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name])
								RageUI.CloseAll()
							else
					...
					if (Selected) then
						...
						if closestDistance ~= -1 and closestDistance <= 3 then
							...
							if not IsPedSittingInAnyVehicle(closestPed) then
								...
								TriggerServerEvent('esx:giSalvadorveInventoryItem', GetPlayerServerId(closestPlayer), 'item_weapon', PersonalMenu.ItemSelected.name, ammo)
								RageUI.CloseAll()
							else
								ESX.ShowNotification(_U('in_vehicle_give', PersonalMenu.ItemSelected.label))
							end
				...
				if Index == 1 then
					...
					if post then
						...
						if closestDistance ~= -1 and closestDistance <= 3 then
							...
							if not IsPedSittingInAnyVehicle(closestPed) then
								TriggerServerEvent('esx:giSalvadorveInventoryItem', GetPlayerServerId(closestPlayer), 'item_money', 'money', quantity)
								RageUI.CloseAll()
							else
						...
						if Index == 1 then
							...
							if post then
								...
								if closestDistance ~= -1 and closestDistance <= 3 then
									...
									if not IsPedSittingInAnyVehicle(closestPed) then
										TriggerServerEvent('esx:giSalvadorveInventoryItem', GetPlayerServerId(closestPlayer), 'item_account', ESX.PlayerData.accounts[i].name, quantity)
										RageUI.CloseAll()
									else
										ESX.ShowNotification(_U('in_vehicle_give', 'de l\'argent'))
									end
