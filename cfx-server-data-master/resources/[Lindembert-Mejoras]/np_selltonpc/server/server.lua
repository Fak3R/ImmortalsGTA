...
AddEventHandler('np_selltonpc:dodeal', function(drugtype)
	...
	if xPlayer ~= nil then
		...
		if inventoryamount >= itemamount then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Tú vendiste ' .. itemamount .. ' ' .. drugtype ..  ' por $' .. moneyamount, length = 4000 })
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'No tienes suficiente ' .. drugtype .. ' para vender.', length = 5000, })
		end
	end
end)
