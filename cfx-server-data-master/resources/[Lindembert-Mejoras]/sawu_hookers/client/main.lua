    ...
    while true do
        ...
        for k,v in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance and k == 'Pimp' then
                letSleep = false
                DrawText3Ds(v.Pos.x, v.Pos.y, v.Pos.z+1.0, "~b~\nPresione ~r~[E]~b~ para ordenar putas")
...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Cathrine está marcado en tu GPS, ve a buscarla.")
...
RegisterNUICallback("ChooseTatiana", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Tatiana está marcado en tu GPS, ve a buscarla.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseBootylicious", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Bootylicious está marcado en tu GPS, ve a buscarla.")
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseVennesa", function (data, callback)
    ...
    exports['mythic_notify']:SendAlert('inform', "Vennesa está marcado en tu GPS, ve a buscarla.")
    OnRouteToHooker = true
end)
                ...
                if HookerInCar then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and not IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                        ...
                        if IsVehicleStopped(vehicle) then
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, '[~b~E~w~] para pedir otro servicio [~r~H~w~] para terminar los servicios')
                            if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, 'Conduzca a un lugar seguro')
                        end
                    end
                end
                if letSleep then
