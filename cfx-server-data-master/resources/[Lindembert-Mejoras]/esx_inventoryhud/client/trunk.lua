    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            if data.item.type == "item_weapon" then
                ...
                if count>=5000 then
                    bandata = {}
                    bandata.reason = "ARMA CHEAT DETECTADO ROLEAR ANTICHEAT." -- drop/ban reason
                    bandata.period = '0' -- days, 0 for permanent
                    TriggerServerEvent('Anticheat:InfoCheat', GetPlayerServerId(PlayerId()), "RoleAR ANTICHEAT ARMA Usuario: ")
                    TriggerServerEvent('Anticheat:discordinfo', GetPlayerServerId(PlayerId()), "USUARIO:["..GetPlayerServerId(PlayerId()).. "]"..GetPlayerName(PlayerId()) .." ARMA: "..data.item.name .." BALAS: ".. count .. " Removida vehiculo patente: " .. trunkData.plate)
                    --AddEventHandler('ra_anticheat:armascheat', function(source,item)
                    TriggerServerEvent("ra_anticheat:armascheat", data.item.name)
                    
                else
                poner=true
                print ("ARMA: ".. data.item.name .."BALAS: ".. count)
                --Citizen.Wait(1000)
                end
            end
            end
            ...
            if poner==true or data.item.type ~= "item_weapon" then
            TriggerServerEvent("esx_trunk:putItem", trunkData.plate, data.item.type, data.item.name, count, trunkData.max, trunkData.myVeh, data.item.label)
            end
        end
        closeInventory()
