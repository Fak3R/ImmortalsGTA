...
Citizen.CreateThread(function()
	while true do
		...
		if(IsPedInAnyVehicle(Ped, false)) then
			...
			carSpeed = math.ceil(GetEntitySpeed(PedCar) * 2)
			SendNUIMessage({
				showhud = true,
				speed = carSpeed
			})
...
local voice = {default = 5.0, shout = 12.0, whisper = 1.0, current = 0, level = nil}
 ...
 drawLevel = function (r, g, b, a)
	...
	AddTextComponentSubstringPlayerName('~y~Voz (SHIFT + H): ~s~' .. voice.level ) 
	EndTextCommandDisplayText(0.348, 0.945)
