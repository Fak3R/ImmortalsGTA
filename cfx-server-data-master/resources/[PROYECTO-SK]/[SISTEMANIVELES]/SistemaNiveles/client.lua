...
RegisterCommand("darexp",function(source, args)
	local FirstParam = args[1]
	TestNR = tonumber(FirstParam)
	XNL_AddPlayerXP(TestNR)
	TriggerServerEvent('AddXp', TestNR)
end)

RegisterCommand("quitarexp",function(source, args)
	local FirstParam = args[1]
	TestNR = tonumber(FirstParam)
	XNL_RemovePlayerXP(TestNR)
	TriggerServerEvent('RemoveXp', TestNR)
end)


--[[
RegisterCommand("darnivel",function(source, args)
	local FirstParam = args[1]
	TestNR = tonumber(FirstParam)
...
Citizen.CreateThread(function()
	while true do
		Wait(120000)  -- One minute (NOTE: Do keep in mind that this isn't a very accurate method though but just an example! :) )
		PlayerOnlineTime = PlayerOnlineTime + 1
		if PlayerOnlineTime == 1 then
			exports.XNLRankBar:Exp_XNL_AddPlayerXP(50)
			exports['mythic_notify']:SendAlert('success', 'Taking a slot 50xp')
...
function CreateRankBar(XP_StartLimit_RankBar, XP_EndLimit_RankBar, playersPreviousXP, playersCurrentXP, CurrentPlayerLevel, TakingAwayXP)
	RankBarColor = 22 -- The Normal Online Ranbar color (IS NOT used for the globes!)
	...
	-- and takes a maximum of 7 parameters which I have described bellow
       BeginScaleformMovieMethodHudComponent(19, "SET_RANK_SCORES")		-- The HUD/Movie Component we want to use to draw the Rankbar
		...
		PushScaleformMovieFunctionParameterInt(150)						-- This one sets the opacity (visibility %) from 0 (invisible) to 100 (fully visible)
