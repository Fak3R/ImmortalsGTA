...
function LoadShop()
	...
	local shopResult = MySQL.Sync.fetchAll('SELECT * FROM weaccesorios')
...
ESX.RegisterServerCallback('esx_accesorios:requestDBItems', function(source, cb)
	if not hasSqlRun then
...
RegisterServerEvent('esx_accesorios:buyItem')
AddEventHandler('esx_accesorios:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('esx_accesorios: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
...
if accountMoney >= price then
		...
		local missingMoney = price -  accountMoney  -- - xPlayer.getMoney()
