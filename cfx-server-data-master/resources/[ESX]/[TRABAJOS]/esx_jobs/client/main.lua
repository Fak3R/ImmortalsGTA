	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			for k,v in pairs(zones) do
				if onDuty or v.Type == "cloakroom" or PlayerData.job.name == "reporter" or PlayerData.job.name == "badabom" then 
		...
		if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' then
			...
			if zones ~= nil then
				...
				if IsControlJustReleased(0, Keys['E']) and not menuIsShowed and isInMarker then
					if onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporter" or PlayerData.job.name == "badabom" then
				...
				if isInMarker and not menuIsShowed then
					hintIsShowed = true
					if (onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporter" or PlayerData.job.name == "badabom" ) and zone.Type ~= "vehdelete" then
						hintToDisplay = zone.Hint
						hintIsShowed = true
					elseif zone.Type == "vehdelete" and (onDuty or PlayerData.job.name == "reporter" or PlayerData.job.name == "badabom") then
						local playerPed = PlayerPedId()
...
RegisterNetEvent("darexperienciadbyhud")
AddEventHandler("darexperienciadbyhud", function(exp)
	asdasd(exp)
end)


function asdasd(exp)
exports['SistemaNiveles']:Exp_XNL_AddPlayerXP(exp)	
TriggerServerEvent('AddXp', exp)
