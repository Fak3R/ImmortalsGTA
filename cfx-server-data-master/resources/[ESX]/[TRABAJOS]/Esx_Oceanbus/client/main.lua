...
Citizen.CreateThread(function()
 ...
 borradopropbuso = function()
	local playerPed  = GetPlayerPed(-1)

print("SE TE BORRO EL PROP")
...
function DestroyObjects(objects)
 ...
 propbuso = function()
		 print("SE TE ACABA DE PONER EL PROPO")
	local playerPed  = GetPlayerPed(-1)
	local coords     = GetEntityCoords(playerPed)

	local boneIndex2 = GetPedBoneIndex(playerPed, 24818)

   local mascara = CreateObject(GetHashKey("p_s_scuba_tank_s"), coords.x,coords.y, coords.z - 3, false, false, false) -- Pillbox Hospital

			AttachEntityToEntity(mascara, playerPed, boneIndex2, -0.30, -0.22, 0.0, 0.0, 90.0, 180.0, true, true, false, true, 1, true)
			SetPedDiesInWater(playerPed, false)
            table.insert(objects, mascara)
...
Citizen.CreateThread(function()
local hash = GetHashKey("cs_carbuyer")
...
if primeraaparicion == true then
	local npc = CreatePed(6,hash, -1600.96, 5204.71, 4.31 -0.95,297.19, false, false)
