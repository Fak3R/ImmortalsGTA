...
ESX.RegisterServerCallback("lindembert-ventas-autos:retrieveVehicles", function(source, cb)
	local src = source
...
ESX.RegisterServerCallback("lindembert-ventas-autos:isVehicleValid", function(source, cb, vehicleProps, price)
	local src = source
	...
	RetrievePlayerVehicles(xPlayer.identifier, function(ownedVehicles)
		...
		for id, v in pairs(ownedVehicles) do
			...
			if Trim(plate) == Trim(v.plate) and #Config.VehiclePositions ~= VehiclesForSale then
                ...
                TriggerClientEvent("lindembert-ventas-autos:refreshVehicles", -1)
...
ESX.RegisterServerCallback("lindembert-ventas-autos:buyVehicle", function(source, cb, vehProps, price)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	...
	if xPlayer.getAccount("bank")["money"] >= price or price == 0 then
		...
		MySQL.Async.fetchAll('SELECT seller FROM vehicles_for_sale WHERE vehicleProps LIKE "%' .. plate .. '%"', {}, function(result)
			if result[1] ~= nil and result[1]["seller"] ~= nil then
				...
				print("Algo salio mal, no habia auto.")
			end
		end)
...
function UpdateCash(identifier, cash)
	...
	if xPlayer ~= nil then
		...
		TriggerClientEvent("esx:showNotification", xPlayer.source, "Alguien compro su vehiculo y le transfirio $/" .. cash)
	else
