    ...
    while true do
        ...
        if inTrunk then
            ...
            if DoesEntityExist(vehicle) or not IsPedDeadOrDying(PlayerPedId()) or not IsPedFatallyInjured(PlayerPedId()) then
                ...
                DrawText3D(coords, '~r~[G] ~w~Salir del maletero')
		...
		then
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 2.0 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                            DrawText3D(coords, '~r~[G]~w~ Entrar al maletero\n~r~[H]~w~ Abrir meletero')
								if IsControlJustReleased(0, 74)then
									if lockStatus == 1 then --unlocked
										SetCarBootOpen(vehicle)
									elseif lockStatus == 2 then -- locked
										ESX.ShowNotification('Carro cerrado')
									end
								end
                        else
                            DrawText3D(coords, '~r~[G]~w~ Entrar al maletero\n~r~[H] ~w~Cerrar maletero')
                            if IsControlJustReleased(0, 74) then
                                SetVehicleDoorShut(vehicle, 5)
                            end
                        end
                    ...
                    if IsControlJustReleased(0, 47) and not inTrunk then
						...
						elseif lockStatus == 2 then -- locked
							ESX.ShowNotification('El carro esta cerrado')
						end
                    end
                end
            end
