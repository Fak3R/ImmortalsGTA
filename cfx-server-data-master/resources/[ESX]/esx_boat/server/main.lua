...
function ParkBoats()
	MySQL.Async.execute('UPDATE owned_botes SET `stored` = true WHERE `stored` = false AND type = @type', {
		['@type'] = 'boat'
...
ESX.RegisterServerCallback('esx_boat:buyBoat', function(source, cb, vehicleProps)
	...
	if price == 0 then
		...
		if xPlayer.getMoney() >= price then
			...
			MySQL.Async.execute('INSERT INTO owned_botes (owner, plate, vehicle, type, stored) VALUES (@owner, @plate, @vehicle, @type, @stored)', {
...
AddEventHandler('esx_boat:takeOutVehicle', function(plate)
	MySQL.Async.execute('UPDATE owned_botes SET `stored` = @stored WHERE owner = @owner AND plate = @plate', {
		['@stored'] = false,
...
ESX.RegisterServerCallback('esx_boat:getGarage', function(source, cb)
	MySQL.Async.fetchAll('SELECT * FROM owned_botes WHERE owner = @owner AND type = @type AND `stored` = @stored', {
		['@owner']  = GetPlayerIdentifiers(source)[1],
		['@type']   = 'boat',
		['@stored'] = true
	}, function(result)
