  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('esx:showNotification', src, '~r~No tienes dinero suficiente.')
            elseif ItemCount <= 0 then
                TriggerClientEvent('esx:showNotification', src, '~r~Cantidad no valida.')
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Has comprado ' .. ItemCount .. 'x ' .. Item .. ' por $' .. ItemCount * result[1].price)
