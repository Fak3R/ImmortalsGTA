...
AddEventHandler('esx_narshop:silencieux', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used < silencieux then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
			     TriggerServerEvent('esx_narshop:removesilencieux')	
		  		 GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey("WEAPON_PISTOL"), GetHashKey("component_at_pi_supp_02")) 				 
		  		 ESX.ShowNotification(("Te acabas de equipar con un silenciador. Tendras que volver a equiparlo cada vez que vuelvas.")) 
		  		 	used = used + 1
	...
	used = used + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede soportar un silenciador."))
					  		 ...
					  		 ESX.ShowNotification(("Tienes que usar todos tus silenciadores.")) 
...
AddEventHandler('esx_narshop:flashlight', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used2 < flashlight then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
			     TriggerServerEvent('esx_narshop:removeflashlight')	
		  		 GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey("WEAPON_PISTOL"), GetHashKey("COMPONENT_AT_PI_FLSH"))  
		  		 ESX.ShowNotification(("Te acabas de equipar con una linterna. Tendrás que volver a equipartela cada vez que vuelvas.")) 
		  		 	used2 = used2 + 1
		  	elseif currentWeaponHash == GetHashKey("WEAPON_PISTOL50") then
			     TriggerServerEvent('esx_narshop:removeflashlight')
	...
	used2 = used2 + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede soportar una lámpara."))
		  		
			end
		else
				  		  ESX.ShowNotification(("Tienes que usar todas tus linternas."))
...
AddEventHandler('esx_narshop:grip', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used3 < grip then
			...
			if currentWeaponHash == GetHashKey("WEAPON_COMBATPDW") then
			     TriggerServerEvent('esx_narshop:removegrip')
		  		 GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey("WEAPON_COMBATPDW"), GetHashKey("COMPONENT_AT_AR_AFGRIP"))  
		  		 ESX.ShowNotification(("Acaba de equiparse con un asa. Tendras que volver a equiparte cada vez que vuelvas...")) 
		  				used3 = used3 + 1
	...
	used3 = used3 + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede soportar un mango."))
		  		
			end
		else
				  		  ESX.ShowNotification(("Tienes que usar todos tus mangos de arma."))
		end
end)
...
AddEventHandler('esx_narshop:yusuf', function(duration)
    ...
    local currentWeaponHash = GetSelectedPedWeapon(ped)
		if used4 < yusuf then
			...
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
			     TriggerServerEvent('esx_narshop:removeyusuf')
		  		 GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey("WEAPON_PISTOL"), GetHashKey("COMPONENT_PISTOL_VARMOD_LUXE"))  
		  		 ESX.ShowNotification(("Acabas de colocar tinte a tu  arma. Tendrás que volver pintarla cuando vuelvas.")) 
		  		 	used4 = used4 + 1
	...
	used4 = used4 + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede soportar."))
		  		
			end
		else
				  		  ESX.ShowNotification(("Has usado todos tus tintes"))
