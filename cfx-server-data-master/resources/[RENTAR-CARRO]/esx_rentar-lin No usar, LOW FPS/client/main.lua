...
if vehicle.job =='police' then
price = vehicle.price
end
if vehicle.job =='mechanic' then
price =vehicle.price
end
...
if spawnedVehs[i].job == 'none' or spawnedVehs[i].job == 'police' or spawnedVehs[i].job == 'mechanic' then 
...
table.insert(elements, { label = "Confirmar alquiler de minutos", value = "rentButton" })
...
ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclesRentrent_this_veh',
	...
	function(data, menu)
        ...
        { 
          title = 'Confirmar alquiler de vehiculo',
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">Alquilar vehiculo por</span> <span style="color:Green;">' .. curRentPrice  .. '$ ?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Confirmar alquiler</span>', value = 'yes'},
            {label = 'Rechazar alquiler', value = 'no'},
          },
        },
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:buyVehicle', function(isBuyed)
				...
				ESX.ShowNotification("~g~Has alquilado el carro exitosamente")
				else 
	  ...
	  elseif not price then 
        ...
        { 
          ...
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">Alquilar transporte?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Confirmar alquiler</span>', value = 'yes'},
            {label = 'Rechazar alquiler', value = 'no'},
          },
        },
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:getVehicle', function()
				...
				ESX.ShowNotification("~g~Has alquilado exitosamente el transporte")
			end, vehicle)
			end
			if data2.current.value == 'no' then
			menu2.close()
        ...
        function (data2, menu2)
	  	...
	  	ESX.ShowNotification("~r~No puede alquilar el vehiculo por 0 minutos")
	  end
	  end
	...
	while true do
		Citizen.Wait(0)
			...
			for i = 1, #spawnedVehs, 1 do
				...
				AddTextComponentString('Presione ~INPUT_CONTEXT~ para alquilar un vehiculo')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0, 38) then
					if rentNoJob >= Config.MaxNoJob then 
					ESX.ShowNotification("No puedes alquilar mas de " .. Config.MaxNoJob .. " vehiculo!")
					else 
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('Presione ~INPUT_CONTEXT~ para tomar el vehiculo')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0, 38) then
				if rentJob >= Config.MaxJob then 
					ESX.ShowNotification("No puede alquilar mas ~y~" .. Config.MaxJob .. " vehiculos!")
					else 
					OpenVehicleMenu(spawnedVehs[i])
				end
				end
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('No puedes alquilar este vehiculo!')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				end
				end
				end
